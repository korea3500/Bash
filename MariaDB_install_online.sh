...
####################################################################################################
# TODO : --basedir, --datadir 등 인자 입력 시, my.cnf 내 값 업데이트 필요.
...
for arg do
	...
	#### --help ####
        if [ "${arg}" == "--help" ] || [ "${arg}" == "help" ]; then
			...
			echo "##################################################################################"
			echo ""
			echo " Please use follow this command : MariaDB_install.sh --<argument>=<value> ..."
			echo " --help "
			echo " --version= "
			# echo " --basedir= "
			# echo " --datadir= "
...
if [ ! -e /home/${user} ]; then
	...
	echo "Please input user password : ${user}.${group}"
	passwd ${user}
...
if [ -e "$(pwd)/${MariaDB_engine}.tar.gz" ] || [ -e "$(pwd)/${MariaDB_engine}" ]; then
	if [ ! -e "${home}/Maria*" ] || [ ! -e "${home}/maria*" ] || [ ! -e "${home}/my.cnf" ]; then	
		echo "[Info] Moved MariaDB files from $(pwd) to ${home}"
		mv Maria* maria* my.cnf ${home}
	...
	if [ ! -e "${basedir}/${MariaDB_engine}.tar.gz" ] && [ ! -e "${basedir}/${MariaDB_engine}" ]; then
		...
		echo "[Warning] ${MariaDB_engine} file is in ${basedir} directory."
		MariaDB_rpms=(${basedir}/MariaDB-*.rpm)
...
if [ -d "${MariaDB_dependencies}" ]; then 
	eval "rpm -Uvh ${MariaDB_dependencies}*.rpm"

else
	echo "[Failed] Cannot find ${MariaDB_dependencies} directory. Please check directory"
	exit 1;
fi
...
if [ -f "${MariaDB_rpms[0]}" ]; then 
	...
	echo "[Failed] Cannot find ${MariaDB_rpms}. Please check your file path"
	exit 1;
...
if [ -d "${basedir}" ]; then
	...
	echo "[Failed] Cannot find ${basedir} directory. Please check your base directory"
	exit 1;
fi
...
if [ -f "/etc/my.cnf" ] && [ -f "${home}/my.cnf" ]; then
	...
	echo "[Info] my.cnf copy successful. Please check your configuration "
fi
...
if [ -e ${engine_dir}/scripts/mariadb-install-db ]; then
	...
	echo "[Failed] System table create failed. you must execute system table script manually. "
fi
